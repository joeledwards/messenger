...
.then(function(context) {
    return db.redis(context.config.redis)
    .then(function(globalRedis) {
...
.then(function(context) {
    ...
    io(context.server).on('connection', function(socket)
    {
        ...
        context.redis.incr('last-client-id', function(error, buffer)
        {
            ...
            var client_alias = b64.encode("" + client_id);
            var client_key = KEY_CLIENT_MAP_PREFIX + client_id;

            socket.emit('welcome', { client_id : client_id });
            console.log("client id: " + buffer);

            return db.redis(context.config.redis).then(function(clientRedis) {
                // Log all channel subscribe actions
                clientRedis.on('psubscribe', function(pattern, count)
                {
                ...
                clientRedis.on('pmessage', function(pattern, channel, message)
                {
                ...
                clientRedis.on('punsubscribe', function(pattern, count)
                {
                    console.log("client " + client_id + 
                ...
                clientRedis.psubscribe(BROADCAST_CHANNEL, direct_channel);
            });
